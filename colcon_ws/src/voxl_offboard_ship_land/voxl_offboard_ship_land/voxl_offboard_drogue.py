...
class OffboardShipLandNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.tag_subscriber = self.create_subscription(
            Float32MultiArray,
            '/yolo/coupler_dx_dist',   # from your UDP->ROS node
    ...
    def tag_pose_callback(self, msg: Float32MultiArray):
        # msg.data = [dx, dist]
        if len(msg.data) >= 2:
            dx = float(msg.data[0])
            dist = float(msg.data[1])
            # Keep same tuple structure expected downstream: (x, y, z)
            # Use x=dx, y=0.0 (unused in your logic), z=dist
            self.tag_pose = (dx, 0.0, dist)
    ...
    def offboard_move_callback(self):
        ...
        if self.tag_pose is not None:
            x, y, z = self.tag_pose 
            x = -x
            horizontal_align = x < 0.45 and x > -0.45
            ready_to_land = horizontal_align and z < 3
